...
Wake PPIs were designed using the LiSBOA tool,\cite{letizia2021lisboa} sweeping approximately 1$^\circ$ of azimuth per second, covering a sector of approximately 20$^\circ$ centered downstream of the turbine. 
...
\comment{Verify scan rates and limits.}
...
\begin{table}[]
  \caption{Description of POD mode structure, periodicity, and associated eigenvalue.}
...
The zeroth mode, $\phi_0$, represents the mean flow, capturing the characteristic momentum deficit in the wind turbine wake (see Table~\ref{tab:pod_modes}).
...
Following \cref{eq:PODset}, we reconstruct the flow field using 16,278 unique subsets including between 3 to 14 POD modes in addition to the mode descibing the mean flow ($\phi_0$).
...
PSDs of wake centers show better agreement at lower frequencies ($f_s < St_{in}$), which the POD modes in ROMs capture more accurately.
...
\label{eq:anova}
\end{equation}
Here, $\overline{\varepsilon}$ represents the global mean coherence, $\beta_i$ are the main effects of individual modes, and $\gamma_{ij}$ capture the interaction effects between modes.
The binary variables $q_i$ indicate the presence (1) or absence (0) of each mode, with the constraint that at least three modes must be included. The term $\xi$ represents the residual error.
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Main effect $\beta_i$ of each mode and interaction effects $\gamma_{ij}$ between modes building the linear system that describes the dependence of average coherence on the presence or absence of each mode. \comment{Does gamma only show absolute values??}}
...
Moreover, modes that have the greatest positive impact on $\varepsilon$, contributing most to the accurate description of wake meandering in the ROM, $\hat{\mu}$, always show distributions with means greater for ROMs that include $\phi_i$ compared to those that exclude $\phi_i$.
...
Recalling that the sample size of ROMs composed with a given number of modes varies (\cref{fig:caseCounts_20240828}), the distributions for ROMs composed with fewer than four or more than 11 modes are not likely to be statistaclly converged.
...
In most cases, the mode $\phi_1$ is qualitatively similar to that shown for 2023-09-04 10:30 UTC in \cref{fig:allModes}, exhibiting a strongly antisymettric character along the average wake centerline and a half-period of streawise variation.
\Cref{fig:beta_allcases} indicates that this mode can account for a huge amount of $\varepsilon$, ranging from a coeffcient of $0.2 \le \beta_1 \le 0.75$, as shown by the pink lines indicating maximum and minimum values.
...
\Cref{fig:beta_allcases} shows (right) the distributions of $\beta_1$ through $\beta_4$ is greater detail as violin plots with markers indicating values for individual cases. 
...
The average colinear dependence of higher order modes is even lower, suggesting that the global behavior for $\varepsilon$ may be that a purely linear realationshipo is sufficient for desciption of wake meandering.
...
While the average behavior of $\beta$ suggests a monotonic decrease in value mode index, the true relationship is likely more complex.
\begin{figure}[h]
  \centering
...
\end{figure}
\Cref{fig:lambda_v_beta} compares the log of the mode eigenvalues, $\log_{10}(\lambda)$ to the main effect coefficients $\beta$, colored by the mode number.
...
POD modes describe the variance-maximizing structures in the observational basis, that is the describe the TKE in the measurement domain of the nacelle-mounted scannig lidars---the eigenvalues $\lambda_i$ quantify the TKE each mode represents in the overall budget.
The main effect coefficients, $\beta_i$ describe the dependence of the streamwise- and reduced frequency-averaged correlation between wake centers estimated by a Gaussian function fit to the observaitonal data and estimates made by a ROM as described by a subset of POD modes.

\comment{Make a figure that compares the dominant frequency of each mode to the inflow strouhal number for each case. Makers can be colored by the respective $\beta$ coefficient. If there's sufficient evidence that modes with high $\beta$ are correlated somehow, try to connect to MO length or other inflow condition.}
...
This study leverages remote sensing data from nacelle-mounted scanning lidars to examine full-scale wind turbine wake dynamics under varying inflow conditions. 
By analyzing observational data from nacelle-mounted scanning lidars, we quantify wake meandering using snapshot POD and spectral methods. 
We find that the wake center`s meandering statistics differ across test cases, revealing insights into the spatiotemporal and spectral characteristics of wake dynamics.

Our analysis of POD modes challenges conventional descriptions of wake meanding and the expected relationships with the atmospheric inflow. 
While low-order modes represent the largest, most energetic turbulent structures, they do not necessarily contribute most to accurate wake meandering representations. 
...
Althoguh quantitatively different from one case to another, the POD modes describing wake dynanics are qualitatively quite similar. 
...
We suspect that a low-order manifold exists that efficiently descirbes wake aerodynamics and wake meandering in a way that can be comuptationally efficient to represent and could benefit steady-state and engineering wind turbine and wind plant wake models/ 

While this study is essentailly a phenomenological desciption of wake meandering and its relationship with turbulent structures describing the wake, it is not a dynamic or predictive model.
...
To bridge this gap, a similar workflow using the classical POD or dynamic mode decomposition could be applied to the timeseries of estiamted wake centers.
By considering the additional dynamics of the turbulent atmospheric inflow and the operating state of turbine, a complete dynamical model for wake centerline trajectories, including wake meandering should be within reach.
